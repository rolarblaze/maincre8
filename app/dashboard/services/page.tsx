...
const Services = () => {
  ...
  return (
    <div className="flex flex-wrap gap-6 xs:max-md:gap-0 xs:max-md:gap-y-6 xs:max-md:justify-between pl-6 py-10 xs:max-md:px-2">
      {bundlesData.map(
        ({ bundle_id, bundle_image_link, bundle_name, description }) => {
          return (
            <Link
              ...
              className={`group w-[30%] min-w-60 overflow-hidden rounded-lg border border-ash xs:max-md:w-[45%] xs:max-md:min-w-56 xs:max-md:mx-auto ${getBundlesClass[bundle_id - 1].tabClass}`}
            >
              <figure
